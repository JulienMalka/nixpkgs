...
def install_uki(bootspec: BootSpec) -> None:
    ...
    for secret in bootspec.stage1_secrets.keys():
        subprocess.check_call(["@systemd@/bin/systemd-creds", f"--name=mdr", "--with-key=tpm2", "--tpm2-device=auto", "encrypt", bootspec.stage1_secrets[secret], f"@efiSysMountPoint@/EFI/Linux/uki.efi.extra.d/mdr.cred"])
